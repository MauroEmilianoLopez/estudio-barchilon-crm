...
async function seed() {
  // Get pipeline stages
  const stagesResult = await client.execute('SELECT id, name FROM pipeline_stages ORDER BY "order"');
  const stages = stagesResult.rows as unknown as Array<{ id: string; name: string }>;

  if (stages.length === 0) {
    console.error(
      "No pipeline stages found. Run 'npm run init' first."
    );
    process.exit(1);
  }

  const stageMap = new Map(stages.map((s) => [s.name, s.id as string]));
  ...
  for (const c of contacts) {
    await client.execute({
      sql: `INSERT OR IGNORE INTO contacts (id, name, email, phone, company, source, temperature, score, notes, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [c.id, c.name, c.email, c.phone, c.company, c.source, c.temperature, c.score, c.notes, c.created_at, c.updated_at],
    });
  }
  ...
  const dealData = [
    {
      ...
      value: 250000,
      stage_id: stageMap.get("Esperando documentos") || stageMap.get("Propuesta") || (stages[2].id as string),
    ...
    },
    {
      ...
      value: 180000,
      stage_id: stageMap.get("En proceso") || stageMap.get("Contactado") || (stages[1].id as string),
  ...
  for (const d of dealData) {
    await client.execute({
      sql: `INSERT OR IGNORE INTO deals (id, title, value, stage_id, contact_id, expected_close, probability, notes, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [d.id, d.title, d.value, d.stage_id, d.contact_id, d.expected_close, d.probability, d.notes, d.created_at, d.updated_at],
    });
  }
  ...
  for (const a of activityData) {
    await client.execute({
      sql: `INSERT OR IGNORE INTO activities (id, type, description, contact_id, deal_id, scheduled_at, completed_at, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [a.id, a.type, a.description, a.contact_id, a.deal_id, a.scheduled_at, a.completed_at, a.created_at],
    });
  }
